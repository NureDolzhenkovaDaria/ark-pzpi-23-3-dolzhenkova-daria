...
router = APIRouter(prefix="/api/v1", tags=["EmoAd System & Admin"])
...
@router.put("/admin/edit_ad/{ad_id}")
async def edit_ad(ad_id: int, ad_data: AdCreate, db: Session = Depends(get_db), _: bool = Depends(verify_admin)):
    ad = db.query(AdvertisementDB).filter(AdvertisementDB.id == ad_id).first()
    if not ad:
        raise HTTPException(status_code=404, detail="Рекламу не знайдено")
    
    ad.emotion = ad_data.emotion.strip().lower()
    ad.promo_code = ad_data.promo_code
    ad.slogan = ad_data.slogan
    db.commit()
    return {"message": "Успішно оновлено", "id": ad.id}
...
async def export_system_data(db: Session = Depends(get_db), _: bool = Depends(verify_admin)):
    ...
    return {
        ...
        "data": {
            "advertisements": [{"id": a.id, "emotion": a.emotion, "slogan": a.slogan, "promo": a.promo_code} for a in ads],
            "users": [{"id": u.user_id, "username": u.username, "role": u.role, "blocked": u.is_blocked} for u in users]
        }
    }
...
async def toggle_user_block(user_id: int, db: Session = Depends(get_db), _: bool = Depends(verify_admin)):
    ...
    status_msg = "заблоковано" if user.is_blocked == 1 else "розблоковано"
    return {"message": f"Користувача успішно {status_msg}", "user_id": user.user_id, "is_blocked": user.is_blocked}